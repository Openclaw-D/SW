# Signal Council · 见微 — V1

Signal Council · 见微 is a local reference workbench for financing-lease material verification and business-rule evaluation. It is not a production service, an automated approval system, a statistically validated risk model, or a repository of real customer material.
...
- Offers three advisory collaboration roles (`business`, `risk`, and `leadership`). Their messages remain advisory and must be formally reviewed by a human before any authoritative action.
...
- There is no real-customer dataset, production authorization model, multitenancy, public OCR/Office service, or verified image-to-3D reconstruction engine.
...
- Front: `201/201` tests passed, plus typecheck and build.
- Back: a fresh run with no external material root passed `419`; `37` frozen offline/oracle modules were skipped because their external native packs are absent; one warning remained.
