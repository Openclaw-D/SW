  ...
  return (
    <div aria-label={copy(locale, "Core cash-flow charts", "流水核心图表")} className="cashflow-core-grid" data-semantic-localized>
      ...
      <section aria-labelledby="cashflow-parties-title" className="cashflow-core-panel cashflow-parties-panel">
        ...
        <div aria-label={copy(locale, "Counterparty Top-5 concentration", "交易对手Top5集中度")} className="cashflow-party-summary"><div><span>{copy(locale, "Inflow parties Top 5", "流入方 Top5")}</span><strong>{inflowConcentration === null ? copy(locale, "Unavailable", "不可用") : `${inflowConcentration.toFixed(1)}%`}</strong></div><div><span>{copy(locale, "Outflow parties Top 5", "流出方 Top5")}</span><strong>{outflowConcentration === null ? copy(locale, "Unavailable", "不可用") : `${outflowConcentration.toFixed(1)}%`}</strong></div></div>
